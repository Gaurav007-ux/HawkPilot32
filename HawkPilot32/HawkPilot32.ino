...
// Special thanks and credit to Joop Brokking for creating and sharing the original 
// open-source YMFC-32 Auto flight controller firmware. This project (HawkPilot32) 
// is a fork of his work, modified and optimized for custom drone integrations.
// ====================================================================================
//
// Safety note:
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
float pid_p_gain_roll = 1.2;               //Slightly lower than default 1.3 — 5.3" props are responsive on light frame.
float pid_i_gain_roll = 0.04;              //Gain setting for the pitch and roll I-controller (default = 0.04).
float pid_d_gain_roll = 15.0;              //Reduced from 18.0 — less D-term noise on light 500–700g frame.
...
float pid_p_gain_yaw = 3.5;                //Reduced from 4.0 — 2200KV motors spin fast, less P needed for yaw.
...
float battery_compensation = 35.0;         //Reduced from 40 — 3S voltage sag is less severe than 4S.

float pid_p_gain_altitude = 1.2;           //Reduced from 1.4 — prevents overshoot on light 500–700g frame.
float pid_i_gain_altitude = 0.15;          //Reduced from 0.2 — slower integral wind-up on responsive frame.
float pid_d_gain_altitude = 0.85;          //Increased from 0.75 — more damping for pressure noise.
int pid_max_altitude = 350;                //Reduced from 400 — limits max throttle correction on light frame.

float gps_p_gain = 2.0;                    //Reduced from 2.7 — less oscillation on light 500–700g frame.
float gps_d_gain = 5.5;                    //Reduced from 6.5 — slightly reduced for 5Hz GPS update rate.

float declination = -0.7;                  //Magnetic declination for northern India — verify at ngdc.noaa.gov.

int16_t manual_takeoff_throttle = 1480;    //3S + 500–700g frame hovers around 1470–1490.
int16_t motor_idle_speed = 1080;           //Slightly lower — 2200KV motors idle well at 1080.

uint8_t gyro_address = 0x68;               //The I2C address of the MPU-6050 is 0x68 in hexadecimal form.
uint8_t BMP280_address = 0x76;             //BMP280 (HW-611) I2C address. Change to 0x77 if SDO pin is HIGH.
uint8_t compass_address = 0x1E;            //The I2C address of the HMC5883L is 0x1E in hexadecimal form.

float low_battery_warning = 10.8;          //3S cutoff — warn at 3.6V/cell (10.8V) for LiPo safety.
...
uint8_t flight_mode_previous;                              //Tracks previous flight_mode for transition detection.
...
int16_t althold_base_throttle;                             //Captured throttle base when alt-hold is first activated.
...
int32_t pressure_rotating_mem[25], pressure_total_avarage;    //Increased from 20 to 25 samples for smoother altitude reference.
...
uint32_t gps_led_timer;                    //Non-blocking millis() timer for GPS LED blink (Task 1).
bool gps_led_state = false;                //Tracks current state of PC13 LED for GPS blink pattern.
...
void setup() {
  ...
  HWire.beginTransmission(BMP280_address);                      //Start communication with the BMP280.
  error = HWire.endTransmission();                              //End the transmission and register the exit status.
  while (error != 0) {                                          //Stay in this loop because the BMP280 did not respond.
  ...
  //Initialize BMP280: read 24 calibration bytes and configure measurement mode.
  setup_bmp280();

  //Let the BMP280 stabilize with 100 readings before flight.
...
void loop() {
  ...
  if (takeoff_detected == 1 && start == 2) {                                         //If the quadcopter is started and flying.
    ...
    if (flight_mode >= 2) {                                                          //If altitude mode is active.
      //On first transition to alt-hold, capture the CURRENT throttle as the base.
      //This prevents the sudden jump from idle (~1000) to hover (~1500).
      if (flight_mode_previous < 2) {
        althold_base_throttle = channel_3 + takeoff_throttle;                        //Start from current throttle, not hardcoded 1500.
        if (althold_base_throttle < motor_idle_speed) althold_base_throttle = motor_idle_speed;
        if (althold_base_throttle > 1700) althold_base_throttle = 1700;
        pid_altitude_setpoint = actual_pressure;                                     //Lock current altitude as hold target.
        pid_i_mem_altitude = 0;                                                      //Reset altitude I-term for clean entry.
        pid_output_altitude = 0;                                                     //Reset altitude PID output.
        manual_altitude_change = 1;                                                  //Indicate manual change to reset parachute tracking.
      }
      //Smoothly ramp althold_base_throttle toward hover (1500 + takeoff_throttle).
      //Rate: 1 us per 4ms loop = 250 us/sec — full ramp takes ~2 seconds.
      int16_t target_hover = 1500 + takeoff_throttle;
      if (althold_base_throttle < target_hover) althold_base_throttle++;
      else if (althold_base_throttle > target_hover) althold_base_throttle--;
      throttle = althold_base_throttle + pid_output_altitude + manual_throttle;      //Use ramped base instead of hardcoded 1500.
    }
  }
  ...
  gps_led_update();                                                                //Update PC13 LED: blink when searching, solid when GPS locked (Task 1).
  ...
  flight_mode_previous = flight_mode;                                              //Remember current flight mode for next loop's transition detection.
