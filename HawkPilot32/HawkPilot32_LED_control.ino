...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// GPS LED status indicator on PC13 (Task 1).
// Called once per main loop iteration (250Hz).
// GPS searching (no fix or < 4 sats): PC13 blinks 500ms ON / 500ms OFF using non-blocking millis() timer.
// GPS locked (>= 4 sats): PC13 goes solid ON (LOW since PC13 is active-low on BluePill).
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
void gps_led_update(void) {
  //Check if we have a valid GPS lock — threshold is >= 4 satellites (stored in number_used_sats, parsed in read_gps.ino).
  if (number_used_sats >= 4) {
    //GPS locked: turn PC13 solid ON (active-low, so LOW = LED on).
    digitalWrite(STM32_board_LED, LOW);
    gps_led_state = true;                                      //Keep state in sync for when lock is lost.
  }
  else {
    //GPS searching: blink PC13 at 500ms ON / 500ms OFF using non-blocking millis() timer.
    if (millis() - gps_led_timer >= 500) {                     //500ms interval for steady, visible blink pattern.
      gps_led_timer = millis();                                //Reset the timer for the next half-cycle.
      gps_led_state = !gps_led_state;                          //Toggle the LED state.
      if (gps_led_state) {
        digitalWrite(STM32_board_LED, LOW);                    //LED ON (active-low).
      }
      else {
        digitalWrite(STM32_board_LED, HIGH);                   //LED OFF (active-low).
      }
    }
  }
}
